...
// --- Validation Helpers & Custom Inputs ---
...
// Input component that only accepts numbers (0-9)
...
// Input component that ensures proper email format and prevents spaces
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      <input
        ...
        onChange={handleChange}
        placeholder={placeholder}
        className={`${className} ${isFilled ? (valid ? "valid" : "invalid") : ""}`}
        required={required}
        autoComplete="email"
      />
      {showStatus && isFilled && (
        <span
          className={`field-validation-msg ${valid ? "success" : "error"}`}
          style={{ display: "block", marginTop: "4px" }}
        >
          {valid ? "✓ Valid email format" : "⚠ Please enter a valid email address (e.g. name@domain.com)"}
        </span>
      )}
    </div>
  );
  ...
  projects: [
    {
      ...
      description: "Automated candidate resume parsing and scoring system using React and NLP.",
      technologies: "React, Python, FastAPI, Tailwind"
    },
    {
      id: 2,
      name: "NextOffer Placement Platform",
      description: "Full-stack student coding and career roadmap ecosystem with resume generator.",
...
function Layout({ currentUser, onLogout, theme, toggleTheme }) {
  ...
  // Determine current breadcrumb from path
  ...
  const userInitials = useMemo(() => {
    const name = currentUser?.profile?.name || currentUser?.name || "VG";
    return name
      .split(" ")
      .map((part) => part[0])
      .slice(0, 2)
        ...
        <Link className="sidebar-user-card" to="/profile">
          <div className="user-card-left">
            ...
            <div className="user-card-details">
              <h4>{currentUser?.profile?.name || currentUser?.name || "Vanshika Goel"}</h4>
              <p>{currentUser?.profile?.title?.split("·")[0] || "Frontend Developer"}</p>
            </div>
          </div>
...
function Dashboard({ currentUser }) {
  ...
  const streak = calculateStreak(currentUser?.activeDates || [getTodayDateString()]);
  const userName = currentUser?.profile?.name || currentUser?.name || "Vanshika";
  ...
  return (
    <section>
      ...
      {/* Hero card with animated streak radar */}
      <div className="dashboard-hero-card">
        <div className="hero-left-content">
          <div className="badge-tag-purple">
            <span>✨</span> YOUR DEVELOPER JOURNEY
          </div>
          <h1>Build skills that<br />compound over time.</h1>
          <p>
            Practice coding every day, ship impactful projects, and turn your technical consistency into job offers.
          </p>
        ...
        <div className="hero-radar-wrapper">
          ...
          <div className="radar-center-badge">
            <span className="streak-num">{streak > 0 ? streak : 17}</span>
            <span className="streak-lbl">Day Streak</span>
          </div>
        </div>
      </div>

      {/* 2-column cards: Daily Challenge & Continue Learning */}
      <div className="dashboard-horizontal-grid">
        <div className="dashboard-panel-card">
          <div className="panel-header-row">
            ...
            <Link to="/topics" className="panel-link-text">
              View all →
            </Link>
          </div>

          <div className="daily-challenge-box" style={{ flex: 1 }}>
            <span className="challenge-number-watermark">01</span>
            <div className="challenge-details">
              <div className="challenge-tags-top">
                <span className="difficulty-badge-easy">EASY</span>
                <span style={{ fontSize: "11px", color: "var(--text-subtle)" }}>{dailyChallenge.acceptance}</span>
              </div>
            ...
            <button
              ...
              onClick={() => navigate("/topics/arrays")}
              title="Solve Problem"
            >
              ➔
            </button>
      ...
      <div className="dashboard-panel-card">
        <div className="panel-header-row">
          <div className="panel-title-group">
            ...
            <p>Your recorded code solutions and status</p>
          </div>
          <Link to="/submissions" className="panel-link-text">
            View all submissions ({submissions.length}) →
          </Link>
        </div>

        {submissions.length === 0 ? (
          <div style={{ padding: "20px 0", color: "var(--text-muted)", fontSize: "13.5px" }}>
            No submissions recorded yet. Head over to the Practice Library to solve your first question!
          </div>
        ) : (
            ...
            {submissions.slice(-3).reverse().map((sub) => (
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: "11.5px", color: "var(--text-muted)" }}>{sub.submittedAt}</p>
                </div>
                <Link
                  to="/submissions"
  ...
  return (
    <section>
      <div style={{ marginBottom: "24px" }}>
        <div className="badge-tag-purple">📝 RECORDED PRACTICE</div>
        <h1 style={{ fontSize: "28px", fontWeight: 800, color: "var(--text-main)", marginBottom: "6px" }}>
          Code Submissions & Solutions
        </h1>
        <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>
          Review your submitted algorithms, execution timestamps, and accepted solutions.
        </p>
      </div>
      ...
      <div className="profile-hero-banner">
        <div className="profile-banner-top">
          <div className="profile-identity-group">
            ...
            <div className="profile-names-group">
              <h2>
                {profile.name || "Vanshika Goel"}
                <span className="verified-badge" title="Verified Developer">✓</span>
              </h2>
              <p className="role-title">{profile.title || "Frontend & Full Stack Developer"}</p>
        ...
        <div className="profile-stats-row">
          <div className="profile-stat-item">
            <strong>{profile.stats?.problems || 87}</strong>
            <span>Problems</span>
          </div>
          <div className="profile-stat-item">
            <strong>{profile.stats?.projects || 12}</strong>
            <span>Projects</span>
          </div>
        </div>
      </div>
      ...
      <div className="profile-details-grid">
        <div className="dashboard-panel-card">
          ...
          <div className="skills-pill-container">
            {(profile.skills || defaultProfile.skills).map((skill) => (
        ...
        <div className="dashboard-panel-card">
          ...
          >
            ...
            <p style={{ fontSize: "13px" }}>No achievements unlocked yet.</p>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleSaveProfile} className="custom-form">
              ...
              <div className="form-field-group">
                <label>
                  ...
                  <span className="field-constraint-tag">Digits only (0-9)</span>
                </label>
                <StrictNumberInput
                  value={editPhone}
  ...
  return (
    <section>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>
          Master essential DSA algorithms, JavaScript concepts, and placement questions.
        </p>
      </div>
...
// --- Page 5: Topic Details & Code Execution ---
                ...
                <div style={{ marginTop: "16px" }}>
                  ...
                  {q.sampleTest && (
                    <div style={{ padding: "8px 12px", background: "var(--bg-app)", borderRadius: "6px", fontSize: "12.5px", marginBottom: "12px", border: "1px solid var(--border-main)" }}>
                      <strong>Test Note:</strong> {q.sampleTest}
                    </div>
                  )}
                  <textarea
                    style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "12px" }}>
                    {feedback[q.id] ? (
                      ...
                      <span style={{ color: "var(--text-subtle)", fontSize: "12px" }}>
                        Submitting updates your streak and logs to Submissions.
                      </span>
                    )}
...
// --- Page 7: Projects & Resume ---

function ProjectsResume({ currentUser, updateUser }) {
  const [projName, setProjName] = useState("");
  const [projDesc, setProjDesc] = useState("");
  const [projTech, setProjTech] = useState("");
  const [contactPhone, setContactPhone] = useState(currentUser?.profile?.phone || "");
  const [contactEmail, setContactEmail] = useState(currentUser?.profile?.email || "");
  ...
  const handleAddProject = (e) => {
    ...
    updateUser({
      ...currentUser,
      projects: [...(currentUser?.projects || []), newP]
    });
  ...
  return (
    <section>
      <div style={{ marginBottom: "24px" }}>
        <div className="badge-tag-purple">📁 PORTFOLIO ENGINE</div>
        <h1 style={{ fontSize: "28px", fontWeight: 800, color: "var(--text-main)", marginBottom: "6px" }}>
          Shipped Projects & Resume
        </h1>
        <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>
          Manage your deployed projects and generate an ATS-optimized technical resume.
        </p>
      </div>
          ...
          <div className="custom-form">
            ...
            <div className="form-field-group">
              <label>
                ...
                <span className="field-constraint-tag">Numbers only (0-9)</span>
              </label>
              <StrictNumberInput
                value={contactPhone}
            ...
            <div style={{ marginTop: "12px", fontSize: "12.5px", color: "var(--text-muted)" }}>
              Contact info will automatically appear in your ATS resume preview below.
            </div>
          </div>
        </div>
      </div>
    ...
    if (mode === "create") {
      ...
      const newUser = {
        ...
        profile: {
          ...
          about: "Enthusiastic developer learning in public and preparing for tech placements.",
          skills: ["JavaScript", "React", "Python", "DSA", "HTML5", "CSS3"],
          stats: {
            problems: 0,
            projects: 0
          }
    ...
    <div className="login-screen-wrapper">
      <div className="auth-form-card">
        <div className="auth-brand-header">
          <div className="brand-icon-box">⚡</div>
          <div>
            <h2 style={{ fontSize: "20px", fontWeight: 800 }}>Next<span>Offer</span></h2>
            <p style={{ fontSize: "12px", color: "var(--text-muted)" }}>Placement Prep Platform</p>
          </div>
        </div>
...
export default function App() {
  ...
  const currentUser = useMemo(() => {
    ...
    return users.find((u) => u.email.toLowerCase() === currentEmail.toLowerCase()) || users[0] || null;
  }, [users, currentEmail]);
    ...
    <Routes>
      ...
      >
        ...
        <Route element={<ProtectedRoute currentUser={currentUser} />}>
          ...
          <Route
            path="topics"
            element={<PracticeLibrary currentUser={currentUser} updateUser={updateUser} />}
          />
          <Route
            path="topics/:topicId"
            element={<TopicDetails currentUser={currentUser} updateUser={updateUser} />}
          />
          <Route
            path="submissions"
            element={<SubmissionsView currentUser={currentUser} />}
          />
          <Route
            path="profile"
          ...
          <Route
            path="projects"
            element={<ProjectsResume currentUser={currentUser} updateUser={updateUser} />}
          />
          <Route
            path="resume"
            element={<ProjectsResume currentUser={currentUser} updateUser={updateUser} />}
